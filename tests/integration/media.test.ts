...
describe("mídia de produto (integração)", () => {
  it("adiciona mídia ao produto", async () => {
    ...
    const media = await addProductMedia(
      productId,
      { type: "IMAGE", url: "https://blob.vercel-storage.com/foto-1.jpg", altText: "Foto 1", position: 0 },
      adminId,
    );
  ...
  it("remove mídia do produto", async () => {
    ...
    const media = await addProductMedia(
      productId,
      { type: "IMAGE", url: "https://blob.vercel-storage.com/foto-2.jpg", position: 0 },
      adminId,
    );
  ...
  it("reordena mídia do produto", async () => {
    const productId = await buildProduct();
    const m1 = await addProductMedia(productId, { type: "IMAGE", url: "https://x/a.jpg", position: 0 }, adminId);
    const m2 = await addProductMedia(productId, { type: "IMAGE", url: "https://x/b.jpg", position: 1 }, adminId);
  ...
  it("define uma mídia como capa (posição 0), preservando a ordem das demais", async () => {
    ...
    const m1 = await addProductMedia(productId, { type: "IMAGE", url: "https://x/a.jpg", position: 0 }, adminId);
    const m2 = await addProductMedia(productId, { type: "IMAGE", url: "https://x/b.jpg", position: 1 }, adminId);
    const m3 = await addProductMedia(productId, { type: "IMAGE", url: "https://x/c.jpg", position: 2 }, adminId);
  ...
  it("atualiza o texto alternativo de uma mídia", async () => {
    const productId = await buildProduct();
    const media = await addProductMedia(productId, { type: "IMAGE", url: "https://x/a.jpg", position: 0 }, adminId);
